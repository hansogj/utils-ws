...
- `find-js`, `abonnement-js`, `maybe` — webpack + ts-loader (`build:wp`), UMD output with `libraryTarget: 'umd'`. The shared `webpack.build.js` at the root provides the rules; each package's `webpack.config.js` extends it with `entry`/`output`. Entry is `src/index.ts` → `dist/index.js` for the first two; `maybe` is the outlier — single-file `src/maybe.ts` → `dist/maybe.js` (filename preserved for the published 2.x contract and the script-tag consumer in `apps/web-js`).
- `array.utils` — **modernized**: `type: "module"`, three-stage build (`tsc` for `.d.ts`, `vite build` for ESM `.js` + CJS `.cjs` per entry, `webpack` for multi-entry UMD bundles (`dist/index.umd.js`, `dist/defined/index.umd.js`, `dist/onEmpty/index.umd.js`, `dist/flatMap/index.umd.js`) — each sub-entry exposes its own `window` global (`window.defined`, `window.onEmpty`, ...) in addition to `window['array.utils']` for the umbrella, for the no-module script-tag consumer in `apps/web-js`). `package.json` `exports` map exposes `.`, `./defined`, `./onEmpty`, `./flatMap` with `types`/`import`/`require`/`default` conditions; legacy `./dist/*` wildcard kept during the transition. `sideEffects` lists the polyfill-bearing entries so consumers can tree-shake the pure ones. Rollup tree-shaking is disabled in `vite.config.ts` (`treeshake: false`) — static analysis can't see that `defined/polyfill.ts` and `onEmpty/onEmpty.ts` mutate `Array.prototype`, and disabling is cheaper than maintaining `/*#__PURE__*/` annotations for a tiny package. This is the template the other published packages will follow as they're modernized.
...
- `apps/web-js` — plain `<script>` tag consumer (port **2112**), no bundler/jest. Its `build` script copies `node_modules/@hansogj` and `shared` into `dist/`, then runs `scripts/inject-deps.cjs` to substitute `<%= dependencies %>` in the HTML with the contents of `harness/shared/src/deps.json` (mirrors what HtmlWebpackPlugin does for web-cs/ts).
...
- `harness/e2e` — Playwright test suite (`pnpm run harness:e2e`) that spins up the three http-servers via Playwright's `webServer` config and asserts: `<h1>` per server, the four-item dependency list rendered as `name@vVERSION` from `deps.json`, expected number of `<pre.success>` entries, no `<pre.error>` anywhere. Catches build-time gotchas the unit tests miss — most notably the tree-shaking-vs-polyfill class that hit during the array.utils modernization.
